...
* Stop generated test methods from carrying the internal marker attribute that ``@parametrize`` sets, which ``functools.wraps()`` copied over from the decorated function.
  Previously, reusing a generated test method in another ``ParametrizedTestCase`` raised a ``TypeError`` about ``@parametrize`` not being the top-most decorator.

* Name the real source in the error for an invalid ID generated by a callable ``ids`` argument.
  Previously, when the callable returned ``None`` and the value's string representation was not a valid Python identifier suffix, the error blamed the callable for a suffix it never returned.

  `PR #186 <https://github.com/adamchainz/unittest-parametrize/pull/186>`__.
