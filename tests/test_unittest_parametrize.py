...
def test_duplicate_param_ids_mixed_reversed():
    with pytest.raises(ValueError) as excinfo:
        parametrize(
            "x",
            [
...
def test_duplicate_single_value_ids():
    with pytest.raises(ValueError) as excinfo:
        parametrize(
            "x",
            [1, 2],
...
def test_duplicate_callable_ids():
    with pytest.raises(ValueError) as excinfo:
        parametrize(
            "x",
            [1, 2],
            ids=lambda value: "same",
        )

    assert excinfo.value.args[0] == "Duplicate param id 'same'"


def test_duplicate_autogenerated_and_explicit_ids():
    with pytest.raises(ValueError) as excinfo:
        parametrize(
            "x",
            [
    ...
    assert excinfo.value.args[0] == "Duplicate param id '1'"
